<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>Web App Dev 2 (2022) @ ACLC Iriga</title>
	<link rel="stylesheet" href="vendor/bootstrap-5.1.3/css/bootstrap.min.css">
	<script src="vendor/jquery-3.6.0/jquery-3.6.0.min.js"></script>
	<link rel="stylesheet" href="css/dark-mode.css">
</head>
<body>

	<!-- App markup -->
	<div class="container mt-3">
		<div align="center">
			<h2><code class="text-primary">Web App Dev2 (2022)</code></h2>
			<p>Practice GitHub Collaboration</p>
			<div class="form-switch">
				<label>
					<input class="form-check-input switch-dark-mode" type="checkbox" role="switch" />
					<small>Dark Mode</small>
				</label>
			</div>
		</div>
		<hr>
		<div class="row">
			<div class="col-md-4 left mb-4"></div>
			<div class="col-md-8 right"></div>
		</div>
	</div>

	<!-- Initialization: DO NOT EDIT -->
    <?php require_once 'includes/init.php'; ?>



    <!-- Add your personal PHP file from 'data' folder -->
    <!-- EXAMPLE -->
    <?php require_once 'data/arvic.php'; ?>

    <!-- ADD IT HERE -->
    <?php // require_once ''; ?>



    <!-- Finalization: DO NOT EDIT -->
    <?php require_once 'includes/fin.php'; ?>
</body>
</html>